...
module.exports = {
  ...
  // 成员在线状态（Presence）
  presenceGraceMs: Number(process.env.PRESENCE_GRACE_MS || 5_000), // 最后一条连接断开后的离线宽限：期内重连不产生状态跳变
  presenceSweepIntervalMs: Number(process.env.PRESENCE_SWEEP_INTERVAL_MS || 1_000), // 宽限到期扫描周期（决定 offline 通知延迟精度）
